...
class Partner(db.Model):
    ...
    timezone = db.Column(db.String(50), default='UTC')  # Partner's preferred timezone
    ...
    subscriptions = db.relationship(
        "PartnerSubscription", backref="partner", lazy="dynamic"
    )
    ...
    def get_pending_subscription(self):
        ...
        return self.subscriptions.filter_by(
            admin_approved=False, status="pending"
        ).first()
...
class PartnerAccessRequest(db.Model):
    ...
    user = db.relationship(
        "User", foreign_keys=[user_id], backref="partner_access_requests"
    )
    reviewer = db.relationship(
        "User", foreign_keys=[reviewed_by], backref="reviewed_partner_requests"
    )
...
class PartnerSubscription(db.Model):
    ...
    approver = db.relationship(
        "User", foreign_keys=[approved_by], backref="approved_partner_subscriptions"
    )

    @property
    def plan_name(self):
        """Get the name of the pricing plan"""
        return self.pricing_plan.name if self.pricing_plan else None
